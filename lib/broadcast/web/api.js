...
module.exports = function () {
  var Channel              = mongoose.model('Channel'),
      ConfiguredChannel    = mongoose.model('ConfiguredChannel'),
      ChannelSet           = mongoose.model('ChannelSet'),
      ConfiguredChannelSet = mongoose.model('ConfiguredChannelSet'),
      Display              = mongoose.model('Display');
  ...
  return {
    ...
    display: {
      create: function (data, callback) {
        ...
        Display.create(data, function (err, display) {
          if (err && err.name === 'ValidationError') { // validation error
            response = new ErrorResponse({ code: 103, data: err.errors });
          } else if (err) { // unknown error
            response = new ErrorResponse({ data: err });
      ...
      read: function (condition, callback) {
        ...
        if (arguments.length === 1) { // if no condition is specified, find all displays
          callback = condition;
          condition = {};
        }
        
        Display.find(condition).populate('channelSets.ref').desc('channelSets.startTime').run(function (err, displays) {
          if (err && err.message === 'Invalid ObjectId') { // if the display doesn't exist
            response = new ErrorResponse({ code: '101' });
          } else if (err) { // unknown error
            response = new ErrorResponse({ data: err });
          } else { // found displays
      ...
      update: function (data, id, callback) {
        ...
        if (typeof id === 'undefined') { // if no ID is given
          ...
          Display.findById(id, function (err, display) {
            ...
            display.updateChannelSets(data.channelSets, function (err, updatedDisplay) {
              if (err) {
                response = new ErrorResponse({ data: err });
              } else {
                response = { display: updatedDisplay };
              }
      ...
      delete: function (id, callback) {
        ...
        if (typeof id === 'undefined') { // if no ID is given
          ...
          Display.findOne({ _id: id }, function (err, display) {
            if (err && err.message === 'Invalid ObjectId') { // if the display doesn't exist
              response = new ErrorResponse({ code: '101' });
            } else if (err) { // unknown error
              response = new ErrorResponse({ data: err });
            } else { // all is good
              return display.remove(function () {
                callback.call(null, { success: true });
              });
            }
